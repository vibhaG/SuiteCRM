    ...
    public function draw($tasks)
    {
        ...
        echo '<table id="Task_table" class="project_table_header">
                <tr class="disable_sort">
                    ...
                    <td style="min-width:85px;" class="project_table_headings">'.$mod_strings['LBL_VISIT_TYPE'].'</td> 
        ...
        if (!is_null($tasks)) {
            foreach ($tasks as $task) {
                ...
                if (ACLController::checkAccess('Project', 'edit', true)) {
                    echo '<td class="project_table_cells" ><span class="Task_name" ><a data = "'.$task->id.','.$task->predecessors.','.$task->relationship_type.','.$timeDate->to_display_date($task->date_start, true).','.$task->duration.','.$task->duration_unit.','.$task->assigned_user_id.','.$task->milestone_flag.','.$task->percent_complete.','.$task->description.','.$task->actual_duration.','.$task->visit_type_c.'" onclick = "edit_task($(this));"title = "'.$mod_strings['LBL_TASK_TITLE'].'" href = "#" >'.$task->name.'</a ></span ></td>';
                } else {
                    echo '<td class="project_table_cells" ><span class="Task_name" >'.$task->name.'</span ></td>';
                }
                 //Urdhvatech
               $visit_type = isset($app_list_strings['visit_type_list'][$task->visit_type_c])? $app_list_strings['visit_type_list'][$task->visit_type_c] : $task->visit_type_c;
                echo '<td class="project_table_cells" ><span class="visit_type" >'.$visit_type.'</span ></td>';
